...
public class ASP extends AbstractPlanner {
    ...
    public static void main(String[] args) {
        try {
            ...
            cmd.execute("domain.pddl" , "p01.pddl");
...
///////////
    ...
    public Plan LocalSearchMCRW(Problem problem){
        ...
        double h_min = current.getHeuristic();

        while(!current.satisfy(G)){

            if(counter > MaxSteps || deadEnd(current, problem)) {
                current = new Node(init, null, -1, 0, 0, heuristic.estimate(init, problem.getGoal()));;
                counter = 0;
            }

            current = MC_Random_Walk(current,problem, heuristic);
            if(current.getHeuristic() < h_min){
                h_min = current.getHeuristic();
                counter = 0;
    ...
    public Node MC_Random_Walk (Node n, Problem p, StateHeuristic h){
        ...
        Node N_min = null;
        double h_min = Double.MAX_VALUE-1;
        ...
        for(int i = 0; i < NUM_WALKS; i++){
            ...
            for (int j = 0; j < LENGTH_WALK; j++){
                ...
                List<Action> a = actionsApplicables(Test_Node, p);
                ...
                State s = new State(Test_Node);
                s.apply(effects);
                Node fils = new Node(s, Test_Node, p.getActions().indexOf(randomAction), n.getCost() + 1, n.getDepth() + 1, 0);
                fils.setHeuristic(h.estimate(fils, p.getGoal()));
                Test_Node = fils;


                if (Test_Node.satisfy(p.getGoal())){
                    return Test_Node;
                }
            }

            if (Test_Node.getHeuristic() < h_min){
                N_min = Test_Node;
                h_min = Test_Node.getHeuristic();
            }
        }
